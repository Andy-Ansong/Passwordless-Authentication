...
// Types for the request body and params
...
const createEmployee = errorHandler(async (req: Request<{}, {}, CreateEmployeeBody>, res: Response) => {
...
const getEmployeeById = errorHandler(async (req: Request<GetEmployeeParams>, res: Response) => {
...
const getAllEmployees = errorHandler(async (req: Request<{}, {}, {}, UserQueryParams>, res: Response) => {
  const searchQuery = req.query.name;
  let query = Employee.find({});
  if (searchQuery) query = search(Employee, { name: searchQuery });
...
const getCurrentEmployee = errorHandler(async (req: Request, res: Response) => {
...
const updateCurrentEmployee = errorHandler(async (req: Request, res: Response) => {
...
const updateEmployeeById = errorHandler(async (req: Request<GetEmployeeParams, {}, UpdateEmployeeBody>, res: Response) => {
  const { Role, Team } = req.body.Department;
  const { position, location, startDate } = Role;
  const { teamName, role, isLeader } = Team;
  ...
  const employee = await Employee.findByIdAndUpdate(
    req.params.id,
    {
      name: req.body.name,
      Department: {
        Role: { position, location, startDate },
        Team: { name: teamName, role, isLeader },
      },
...
const deleteEmployeeById = errorHandler(async (req: Request<GetEmployeeParams>, res: Response) => {
  const employeeId = req.params.id;
  const employee = await Employee.findById(employeeId).exec();
  if (!employee) {
    return res.status(404).send({
...
const getAllEmployeesInTeam = errorHandler(async (req: Request, res: Response) => {
...
// others
const bookALeave = errorHandler(async (req: Request, res: Response) => {});
const approveLeave = errorHandler(async (req: Request, res: Response) => {});
const rejectLeave = errorHandler(async (req: Request, res: Response) => {});
